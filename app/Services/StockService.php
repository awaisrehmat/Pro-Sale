...
class StockService {
    public function move(Product $product, float $in, float $out, string $type, object $reference, string $number, float $cost, int $userId, ?string $notes=null): void {
        ...
        StockMovement::create(['product_id'=>$product->id,'movement_date'=>now()->toDateString(),'movement_type'=>$type,
    ...
    public function adjust(array $data,int $userId): StockAdjustment {
        return DB::transaction(function()use($data,$userId){
            ...
            $this->move($p,$increase?(float)$data['quantity']:0,$increase?0:(float)$data['quantity'],
                $increase?'positive_adjustment':'negative_adjustment',$a,'ADJ-'.str_pad($a->id,6,'0',STR_PAD_LEFT),(float)$p->average_cost,$userId,$data['reason']);
            return $a;
        });
    }
}
