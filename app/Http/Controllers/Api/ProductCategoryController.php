...
class ProductCategoryController extends Controller {
    ...
    public function storeCategory(Request $r){$d=$r->validate(['name'=>['required','string','max:100',TenantRule::unique('product_categories','name')],'is_active'=>'boolean']);return $this->ok(ProductCategory::create($d),'Category created.',201);}
    public function updateCategory(Request $r,ProductCategory $category){$d=$r->validate(['name'=>['required','string','max:100',TenantRule::unique('product_categories','name')->ignore($category)],'is_active'=>'boolean']);$category->update($d);return $this->ok($category->load('subcategories'),'Category updated.');}
