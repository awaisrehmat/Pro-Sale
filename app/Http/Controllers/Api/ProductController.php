...
class ProductController extends Controller {
    ...
    public function index(Request $r){$q=Product::query()->when($r->search,fn($q,$s)=>$q->where(fn($x)=>$x->where('name','like',"%$s%")->orWhere('sku','like',"%$s%")->orWhere('barcode','like',"%$s%")))
    ...
    public function show(Product $product){return $this->ok($product->load(['stockMovements'=>fn($q)=>$q->latest('movement_date')]));}
