...
const open=ref(false),query=ref(''),active=ref(0),input=ref(null);
...
function focus(){if(props.disabled)return;query.value='';open.value=true;nextTick(()=>input.value?.select())}
...
function typed(){open.value=true;if(selected.value&&query.value!==selected.value.label)emit('update:modelValue','')}
...
<template><div class="search-select" :class="{open,disabled}"><input ref="input" v-model="query" :placeholder="placeholder" :disabled="disabled" :required="required&&!modelValue" autocomplete="off" role="combobox" :aria-expanded="open" @focus="focus" @click="focus" @input="typed" @keydown="keydown" @blur="close"><span class="search-select-chevron" aria-hidden="true"></span><div v-if="open" class="search-select-menu"><button v-for="(option,index) in filtered" :key="option.value" type="button" :class="{active:index===active,selected:String(option.value)===String(modelValue)}" @mousedown.prevent="choose(option)"><span>{{option.label}}</span><small v-if="option.meta">{{option.meta}}</small></button><div v-if="!filtered.length" class="search-select-empty">No matching options</div></div></div></template>
